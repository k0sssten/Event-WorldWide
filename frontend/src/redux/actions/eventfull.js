...
export const getEventsByParams = (formData) => async (dispatch) => {
  ...
  const eventsByParams = await axios(
    `https://app.ticketmaster.com/discovery/v2/events.json?apikey=${apikey}&countryCode=${countryCode}&startDateTime=${startDateTime}&city=${city}&classificationName=${classificationName}`
  );
